...
__export(react_exports, {
  useWebMCPAvailable: () => useWebMCPAvailable,
  useWebMCPKit: () => useWebMCPKit,
  useWebMCPLog: () => useWebMCPLog,
...
// src/react.ts
var _sharedKit = null;
function getSharedKit(config) {
  if (!_sharedKit) {
    _sharedKit = createKit(config);
  }
  return _sharedKit;
}
function useWebMCPTool(tool, config) {
  const kit = (0, import_react.useMemo)(() => getSharedKit(config), []);
  const registeredRef = (0, import_react.useRef)(false);
...
function useWebMCPTools(tools, config) {
  const kit = (0, import_react.useMemo)(() => getSharedKit(config), []);
  (0, import_react.useEffect)(() => {
...
function useWebMCPLog(maxEntries = 100) {
  const [invocations, setInvocations] = (0, import_react.useState)([]);
  const kit = (0, import_react.useMemo)(
    () => getSharedKit({
      onAfterInvoke: (event, result) => {
        setInvocations(
          (prev) => [{ ...event, result }, ...prev].slice(0, maxEntries)
        );
      }
    }),
    []
  );
  const clearLog = (0, import_react.useCallback)(() => setInvocations([]), []);
  return { invocations, clearLog };
}
function useWebMCPKit(config) {
  return (0, import_react.useMemo)(() => getSharedKit(config), []);
}
